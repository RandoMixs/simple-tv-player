...
var video = document.querySelector('video'), canais, canal = null, fast = false, ultrafast = false, debug = false, volume = 100;

if(navigator.userAgent.includes('Tizen')) { // Tizen is slowly, optimize for the TV by disabling animations!
    // ultrafast = true; // If the TV is still slow, force disable all CSS animations
    fast = true;
}
...
if(ultrafast) {
    ...
    console.log('Ultra fast mode enabled, all CSS animations disabled!');
}

var hls = new Hls();
function play(url) {
    $('.err').css('display','none');
    $('.loader-video').fadeIn(300);
    if (Hls.isSupported()) {
        hls.destroy();
        hls = new Hls();
        hls.loadSource(url);
        ...
        hls.on(Hls.Events.ERROR, function (event, data) {
            if(data.fatal) {
                $('video').css('opacity','0');
                $('.loader-video').fadeOut(200).promise().then(() => $('.err').fadeIn());
            }
        });
    ...
    $(video).one('canplay',()=>{
        ...
        if (playPromise !== undefined) {
            playPromise.then(function() {
                $(video).animate({ volume: (volume / 100) }, 1000);
            }).catch(function(error) {
                video.muted = true;
                video.play();
            });
        }
        $('.loader-video').fadeOut(200).promise().then(() => {
            $(video).css('opacity', '1');
        });
    });
}
...
$.getJSON('https://api.randomixs.com/tv', (data) => {
    ...
    $('#loading').fadeOut(350, function () {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => play($('.canais.opc:eq(0)').attr('data-url')), 200);
        }, 200);
    });
    canal = 0;
...
$(document).on('keydown', (event) => {
    ...
    if(forcewait) return;
    if(!forcewait) setTimeout(function () {
        clearTimeout(rst);
        forcewait=false, rst = setTimeout(function () {
            $('.canais.opc .loader').removeClass('notransition');
        },250)
    },60);
    forcewait = true;
    ...
    clearTimeout(b);
    b = setTimeout(()=>{
        $('video').css('opacity', 0).animate({volume: 0}, 400).promise().then(()=>{
